...
public class CardsUtil {
    ...
    /** 正则表达式：验证台湾居民居住证 新版18位数字,830000开头 */
    public static final String REGEX_TW_DJCARD = "^830000(?:19|20)\\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\\d|3[01])\\d{3}[\\dX]$";
    /** 正则表达式：验证港澳居民居住证 18位数字,810000 820000开头 */
    public static final String REGEX_HK_DJCARD = "^8[12]0000(?:19|20)\\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\\d|3[01])\\d{3}[\\dX]$";
    ...
    public static boolean isIDCard(String idCardNo) {
        ...
        *
        *
        * 通过字典，部门啊等等方法   校验数据是否符合规范
        *
        * 3.不符合规范的直接返回并不用详细的提示错误信息
        *
        * 4.返回后罗杰和原来正常操作的逻辑相同
        *
        *
    ...
    public static boolean isDriveCard(String driveNo) {
        //校验非空 校验长度
        if (StringUtils.isEmpty(driveNo) || driveNo.length() != 12 ) {
            return false;
        }
    ...
    public static boolean isTWDJCard(String TWNo) {
        //校验非空
        if (StringUtils.isEmpty(TWNo)) {
            return false;
        }
        return TWNo.matches(REGEX_TW_DJCARD);
    }
    public static boolean isHKDJCard(String TWNo) {
        //校验非空
        if (StringUtils.isEmpty(TWNo)) {
            return false;
        }
        return TWNo.matches(REGEX_HK_DJCARD);
    }
